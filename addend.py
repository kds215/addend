...
# KDS 11/18/23 changed to optional different output filename,
...
if args.version:
    print(f"\n   >>> {scriptname} running version: {isVersion}\n")
    exit()
# :end: if
...
if inFilename.lower().endswith(".py"):
    ...
    # Load the python file lines and removes old :end: lines too
    ...
    # in case of NO (optional) output file rename input to backup filename
    if not outFilename:
        ...
        # :end: if
            ...
            print(
                f"\n   >>> file ERROR: {scriptname} is missing '{inFilename}' ; check --help \n"
            )
            exit()
        # :end: else:
    # :end: if
    ...
    # :end: if
        ...
        print(
            f"\n   >>> failed:  {scriptname} output file='{endfile}' does not exist. Unable to rename to '{inFilename}' \n"
        )
        print(
            f"\n   >>> see original input file: '{backupFilename}' for possible syntax conflicts. \n"
        )
...
# :end: if
    ...
    print(
        f"\n   >>> syntax error expecting: {scriptname} 'inFilename.py' ; check --help for details.\n"
    )
# :end: else:
