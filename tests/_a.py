...
def load_input(filename):
    ...
    if isPlatform == "Windows":
        success_code = subprocess.call(
            ["black", "-C", "-S", "-q", filename], shell=True
        )
    # :end: if
    else:
...
def main():
    ...
    parser.add_argument(
        "-d", "--debug", action="store_true", help="print debugging lines."
    )
    parser.add_argument(
        "-v", "--version", action="store_true", help="print version number."
    )
    parser.add_argument(
        "inFilename",
        type=str,
